...
def mock_info_client():
    ...
    mock.user_state = Mock(return_value={
        "assetPositions": [],
    ...
    mock.spot_meta = Mock(return_value={
        "tokens": [
            {"name": "PURR", "szDecimals": 2, "index": 0},
            {"name": "HYPE", "szDecimals": 2, "index": 1},
        ]
    })
    mock.meta_and_asset_ctxs = Mock(return_value=[
        [
            {
                "coin": "BTC",
                "markPx": "50000",
...
def mock_exchange_client():
    ...
    mock.cancel = Mock(return_value={
        "status": "ok",
        "response": {"type": "cancel", "data": {"statuses": ["success"]}}
    })
    mock.cancel_all = Mock(return_value={
        "status": "ok",
...
async def mock_decimal_manager(mock_info_client) -> DecimalPrecisionManager:
    ...
    # Mock asset metadata
    manager.get_asset_metadata = AsyncMock(return_value=Mock(
        symbol="BTC",
        asset_type="perp",
        sz_decimals=4,
        max_decimals=6,
